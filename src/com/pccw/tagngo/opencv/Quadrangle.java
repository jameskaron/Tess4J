...
class Line {
	...
	public static Point intersect(Line l1, Line l2) {
		return getLineLineIntersection(l1.getStart().x, l1.getStart().y, l1.getEnd().x, l1.getEnd().y, l2.getStart().x,
				l2.getStart().y, l2.getEnd().x, l2.getEnd().y);
	}

	public static Point getLineLineIntersection(double x1, double y1, double x2, double y2, double x3, double y3,
			double x4, double y4) {
...
class Quadrangle {
	static int TOP = 0, RIGHT = 1, BOTTOM = 2, LEFT = 3;
	...
	public Mat warp(Mat src) {
		...
		srcProjection[0] = Line.intersect(lines[BOTTOM], lines[RIGHT]);
		srcProjection[1] = Line.intersect(lines[BOTTOM], lines[LEFT]);
		srcProjection[2] = Line.intersect(lines[TOP], lines[RIGHT]);
		srcProjection[3] = Line.intersect(lines[TOP], lines[LEFT]);
